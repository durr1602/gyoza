...
def generate_mutants(wtseq_path, outpath, codon_table, codon_mode):
    ...
    for mutated_seq in wt_df["Mutated_seq"].unique():
        ...
        if len(sub_df["WT_seq"].unique()) > 1:
            raise ValueError(f"Error.. For any mutated sequence, there should be a single possible WT sequence.")
        ...
        expmut_df = pd.concat([wt_df, mutants_df.drop(["mutations"], axis=1)], ignore_index=True)
...
generate_mutants(snakemake.input[0],
                 snakemake.output[0],
