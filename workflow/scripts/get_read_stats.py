...
def generate_read_stats(
    cutadapt_logfiles, pandaseq_logfiles, vsearch_logfiles, outpath
):
    ...
    for f in cutadapt_logfiles:
        ...
        if "This is cutadapt 5.0" not in lines[0]:
            raise Exception(
                f"Error.. {f} is not properly formatted. Make sure you've added the --use-conda flag in the snakemake command line, which specifies the correct package versions to be used"
            )
